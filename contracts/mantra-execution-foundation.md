...
**Requirement contribution:** Verify and record the single-upload batched GPU NMF execution used to build the K562 control-program bank.
...
| <nobr><code>E0-REQ-04</code></nobr> | The K562 control-program pipeline must fit every declared NMF restart concurrently from one preloaded dense nonnegative float32 residual tensor, execute batched multiplicative updates on the GPU, and use seeded CPU Lloyd KMeans for deterministic consensus. It must retain the accepted rank, programs, coefficients, reconstruction diagnostics, backend, elapsed time, input-transfer count and bytes, and peak GPU memory. | in_progress | <nobr><code>E0-VR-04</code></nobr> | <nobr><code>E0-PB-04</code></nobr> |
...
| <nobr><code>E0-VR-04</code></nobr> | <nobr><code>E0-REQ-04</code></nobr> | The GPU update kernels match the CPU reference within the declared tolerance for identical initialization; the accepted control-program run loads the dense residual matrix onto the device exactly once, executes every restart concurrently with finite float32 factors, emits 19 non-collapsed programs through seeded CPU Lloyd KMeans consensus, and retains elapsed time, transfer bytes, and peak GPU memory. A changed transfer count, dtype, backend, consensus profile, numerical result, or missing execution diagnostic fails. | [test_gpu_cnmf_matches_reference_and_records_execution](../../mantra-rebuild/tests/test_control_programs.py) | [test_rejects_changed_gpu_cnmf_execution_record](../../mantra-rebuild/tests/test_control_programs.py) |
