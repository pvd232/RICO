...
| --- | --- | --- | --- | --- |
| K562 control-program cNMF and NMF | `src/mantra/sota/programs/discovery/cnmf.py::run_cnmf`; `nmf_torch_batched.py::nmf_torch_batched` | One dense float32 residual-tensor upload, concurrent GPU restarts, compiled batched tensor updates | Compare identical update tensors on CPU and GPU. Use seeded CPU Lloyd KMeans for deterministic consensus. Retain elapsed time, transfer count and bytes, and peak GPU memory. | Control-program construction |
